...
export default function App() {
      ...
      .then((data) =>
        setQuiz(() => {
          return data.results.map((details) => {
            ...
            let options = shuffle([...wrong, right]);
  ...
  const buttonElement = showAnswers ? (
    <div className="btn-container">
      <span className="result">
        You got {score} out of {formData.number} answers correctly ({percentage}
        %)
      </span>
      <button className={`main-btn btn ${mode}`} onClick={resetQuiz}>
        Play Again
      </button>
    </div>
  ) : (
    <div className="btn-container">
      <button className={`main-btn btn ${mode}`} onClick={checkAnswer}>
        Check Answers
      </button>
    </div>
  );
  ...
  function isSelected(id, aId) {
    const make = quiz.map((prevData) => {
      ...
      return prevData.id === aId ? { ...prevData, answers: pls } : prevData;
    });
    setQuiz(make);
  }
